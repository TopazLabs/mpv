...
namespace aiutils {
    ...
    private:
        ...
        std::ostream _nullStream{new NullBuffer()};
        std::ostream _outStream{new NullBuffer()};
        std::mutex _mutex;

        inline static Logger* __pLogger = nullptr;

        Logger() {};
        explicit Logger(std::streambuf* pStreamBuffer):_isOutStreamSet(true), _outStream(pStreamBuffer) {}
