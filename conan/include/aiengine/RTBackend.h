...
namespace aiengine {
    ...
    private:
        ...
        bool _fixInputNames = false, _fixOutputNames = false;
    ...
    public:
        ...
        virtual void printDescription() override {
            ...
            for (auto& kv: _tensorInfos)
                AIE_INFO() << (kv.second.isInput ? " Input: " : " Output: ")
                    << kv.second.name.c_str()  << " " << kv.second.size  << " " << kv.second.dims.nbDims;
        }
    ...
    protected:
        cudaStream_t _pComputeStream = nullptr, _pDownloadStream = nullptr, _pUploadStream = nullptr;
    };
}
