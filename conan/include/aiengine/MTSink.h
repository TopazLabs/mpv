...
namespace aiutils {
    ...
    class MTSink {
    protected:
        unsigned int _threadCount = 1;
        atomic_bool _keepProcessing = false;
        unique_ptr<BlockingQueue<T>> _pInputQueue;
        vector<thread> _threads;
        atomic_uint _activeProcesses = 0;
    ...
    public:
        explicit MTSink(unsigned int maxQueueLength = 0, unsigned int threadCount = 1): _threadCount(threadCount),
                _pInputQueue(maxQueueLength ? new ClampedBlockingQueue<T>(maxQueueLength) : new BlockingQueue<T>()) {}
