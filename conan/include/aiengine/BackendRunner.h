...
namespace aiengine {
    ...
    protected:
        ...
        MODEL_BACKEND_TYPE _modelType = MB_NONE;
        vector<pair<int,int>> _deviceIds;
        Json _modelBackendInfo, _modelInfo;
        bool _switchInputBackends = false;
        ...
        ModelBackend* makeModelBackend(int deviceId) {
            ...
            if (!pBackend->loadFromFile(_modelPath, _modelKey)) {
                delete(pBackend);
                if(fs::exists(fs::path(_modelPath)) && !ModelManager::GetInstance().allowDownloads()) {
                    AIE_CRITICAL() << "Model file exists but cannot be loaded and downloads are not allowed " << _modelPath;
                    return nullptr;
                }
