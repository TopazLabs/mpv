...
namespace aiengine {
    ...
    private:
        ...
        int _preferredDevice = 0;
        int _allBackends = 0;
        static unique_ptr<DeviceSelector> __pDeviceSelector;

        explicit DeviceSelector(bool hasGLModels = true):_systemInfo(SystemInfo::GetInstance()),
                _devices(_systemInfo.getGPUList()), _totalRAM(_systemInfo.cpuMemory().first),
                _preferredDevice(findPreferredDevice()), _allBackends(AllBackends()) {}
