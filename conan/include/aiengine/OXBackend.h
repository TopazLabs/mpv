...
namespace aiengine {
    ...
    private:
        OrtSession* _pSession = nullptr;
        OrtSessionOptions* _pSessionOptions = nullptr;
        OrtRunOptions* _pRun = nullptr;
        OrtCUDAProviderOptionsV2* _pCudaOptions = nullptr;
        static OrtMemoryInfo* __pMemoryInfo;
        OrtAllocator* _pAllocator = nullptr;
        vector<string> _outputNames;
        map<string, pair<ONNXTensorElementDataType, vector<int64_t>>> _inputInfo;
        bool _fixInputName = false, _fixOutputName = false, _fp16 = false;
        ...
        void initModelInfo() {
            ...
            if (CheckStatus(__pOrtApi->SessionGetInputCount(_pSession, &count), "Input Count: ")) {
                for (size_t i = 0; i < count; i++) {
                    ...
                    vector<int64_t> dims;
                    dims.resize(n);
                    std::ignore = __pOrtApi->GetDimensions(pTensorInfo, (int64_t*)dims.data(), n);
        ...
        static OrtEnv* Env() {
            ...
            if (__pEnv == nullptr) {
                ...
                std::ignore = OrtGetApiBase()->GetApi(ORT_API_VERSION)->CreateEnv(ORT_LOGGING_LEVEL_VERBOSE, "OXBackend", &__pEnv);
#else
                std::ignore = OrtGetApiBase()->GetApi(ORT_API_VERSION)->CreateEnv(ORT_LOGGING_LEVEL_ERROR, "OXBackend", &__pEnv);
#endif
            }
