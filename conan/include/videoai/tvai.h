...
extern "C" {
     ...
     *
     * @return The license type, 0 for Floating, 1 for Fixed.
     */
    DECL_EXPORT int tvai_license_type();

    /**
     * Sends a heartbeat to the license server.
     *
     * @return 0 if successful, -1 otherwise.
     */
    DECL_EXPORT int tvai_license_heartbeat();

    /**
     * Gets the username of the currently logged in user.
     *
     * @param username Buffer to store the username.
     * @param n Maximum length of the username buffer.
     * @return 0 if failed, username length if successful.
     */
    DECL_EXPORT int tvai_get_username(char* username, int n);
